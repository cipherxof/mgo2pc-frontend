...
export class WebAPI {
  ...
  public async createAccount(
    ...
    return await this.makeRequest(`/api/v1/register`, 'post', {
      username,
      password,
      email,
      captcha,
  ...
  public async getRankings(mode: string, page: number, weekly?: boolean): Promise<any | false> {
    return await this.makeRequest(
      `/api/v1/rankings/${mode}/${page}/${weekly ? 'weekly' : ''}`,
      'get',
    );
  }
  ...
  public async getShopItems(
    token: string,
    id?: string,
  ): Promise<WebAPIResponse<{ items: ShopItem[] }> | false> {
  ...
  public async getNotifications(
    token: string,
  ): Promise<WebAPIResponse<{ notifications: UserNotification[] }> | false> {
  ...
  public async getGames(): Promise<
    WebAPIResponse<{ players: number; lobbies: GameLobby[] }> | false
  > {
